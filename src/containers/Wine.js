...
function Wine(props) {
   ...
   let yearBreakDownData,
      varietyBreakdownData,
      regionBreakdownData,
      yearAndVarietyBreakdownData = [];
   ...
   if (wine) {
      ...
      yearAndVarietyBreakdownData = getBreakDownData(lotCode, 'year', 'variety')
         .breakdown;
   }

   if (loading) {
      return <LoadingOutlined style={{ fontSize: '36px' }} />;
   }
   return (
      <Theme>
         <Helmet>
         ...
         {wine ? (
            <>
               ...
               <Descriptions title='Wine Information' bordered>
                  ...
                  {wine.description && (
                     <Descriptions.Item label='Product state'>
                        {wine.description}
                     </Descriptions.Item>
                  )}
